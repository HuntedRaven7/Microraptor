...
* every source ``MatchPattern`` corresponds to an artifact name that some
  element under ``elements/`` actually emits (``microraptor-ddi-<v>.raw.zst``,
  ``microraptor-<v>.efi``, ``k0s-<v>.raw.zst``)
* the k0s sysext transfer lands in ``/var/lib/k0s`` under a name that
...
RELEASE_FEED = "https://github.com/HuntedRaven7/Microraptor/releases/latest/download/"
...
def test_sysupdate_directories_are_populated():
    ...
    assert generic == ["10-microraptor.transfer", "50-root.transfer", "60-uki.transfer"]
...
def test_every_source_artifact_is_staged_in_release_workflow(path: Path):
    """The release asset a transfer downloads must be staged into dist/release/ by release.yml."""
    prefix, _ = split_match_pattern(
        load_transfer(path)["Source"]["MatchPattern"]
    )
    release_yml = (REPO_ROOT / ".github" / "workflows" / "release.yml").read_text()
    assert re.search(rf"cp\s+.*{re.escape(prefix)}\*.*dist/release/", release_yml), (
        f"{path.name} source asset prefix {prefix!r} is not staged to dist/release/ in .github/workflows/release.yml"
    )


def test_k0s_release_staging_does_not_use_legacy_k3s_name() -> None:
    release_yml = (REPO_ROOT / ".github" / "workflows" / "release.yml").read_text()
    assert re.search(r"cp\s+.*k0s-\*\.raw\.zst.*dist/release/", release_yml)
    assert not re.search(
        r"cp\s+.*k3s-\*\.raw\.zst.*dist/release/", release_yml
    )