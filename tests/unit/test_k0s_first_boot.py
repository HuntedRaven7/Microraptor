...
ELEMENT = ROOT / "elements" / "microraptor" / "os-k0s-first-boot.bst"
K0S_UPDATE_ELEMENT = (
    ROOT / "elements" / "microraptor" / "os-k0s-sysupdate.bst"
)
...
def test_k0s_first_boot_is_packaged_and_enabled() -> None:
    ...
    assert (
        "microraptor/os-k0s-first-boot.bst"
...
def test_k0s_sysupdate_transfer_is_packaged_as_a_component() -> None:
    ...
    assert (
        "microraptor/os-k0s-sysupdate.bst"
        in STACK.read_text(encoding="utf-8")
    )